...
namespace MvcNetCorePaginacionRegistros.Controllers
{
    public class PaginacionController : Controller
    {
        ...
        public async Task<IActionResult> DetallesDepartamento(int iddepartamento, int? posicion)
        {
            if (posicion == null)
            {
                posicion = 1;
            ...
            ModelEmpleadosDepartamento modelo = await this.repo.GetEmpleadosDepartamentoAsync(posicion.Value, iddepartamento);
            if (modelo.Empleados.Count == 0)
            {
                modelo.Posicion = 0;
            }
            int siguiente = posicion.Value + 1;
            if (siguiente > modelo.NumeroRegistros)
            {
                siguiente = modelo.NumeroRegistros;
            }
            ...
            ViewData["POSICION"] = modelo.Posicion;
            ViewData["ULTIMO"] = modelo.NumeroRegistros;
        ...
        public async Task<IActionResult> PaginarGrupoVistaDepartamento(int? posicion)
        {
            ...
            // Ahora debemos dibujar números de página
            // que dependen del número de registros
            int numeroPagina = 1;
            int numRegistros = await this.repo.GetNumeroRegistrosVistaDepartamentosAsync();
